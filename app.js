...
const views={calculator:document.querySelector('form#calculatorForm'),arrears:document.getElementById('arrearsView'),tables:document.getElementById('tablesView'),readme:document.getElementById('readmeView')};
function showView(name){
  ...
  if(name==='arrears'){syncArrears();calculateArrears();}
}
...
const factors=[1.039,1.122,1.212,1.309,1.413,1.526,1.649,1.781,1.923,2.077,2.243,2.423,2.616,2.826,3.052,3.296,3.560,3.845,4.152,4.485,4.843,5.231,5.649,6.101,6.589,7.117,7.686,8.301,8.965,9.682,10.487,11.294,12.197,13.173,14.227,15.36555,16.59509,17.92303,19.35722,20.90618,22.57909,24.38586];
document.getElementById('tableBBody').innerHTML=factors.map((v,i)=>`<tr><td>${i}</td><td>${v.toFixed(3)}</td></tr>`).join('');
...
function syncArrears(){
 document.getElementById('arrearsPension').value=Math.round(parseFloat(document.getElementById('monthlyPension').textContent.replace(/[₹,]/g,''))||0);
 const rowExits=Array.from(document.querySelectorAll('#serviceTable .exit')).map(x=>d(x.value)).filter(Boolean);
 const lastExit=rowExits.length?new Date(Math.max(...rowExits.map(x=>x.getTime()))):null;
 document.getElementById('arrearsStart').value=lastExit?isoDate(addDays(lastExit,1)):'';
 document.getElementById('arrearsCutoff').value=endOfCurrentMonthISO();
 calculateArrears();
}
function calculateArrears(){
 const pension=+document.getElementById('arrearsPension').value||0;
 const startValue=document.getElementById('arrearsStart').value;
 const cutoffValue=document.getElementById('arrearsCutoff').value;
 const start=startValue?new Date(startValue+'T00:00:00'):null;
 const cutoff=cutoffValue?new Date(cutoffValue+'T00:00:00'):null;
 if(!start||!cutoff||isNaN(start)||isNaN(cutoff)||!pension||cutoff<start){
  ['partMonthAmount','fullMonthAmount','totalArrears'].forEach(id=>document.getElementById(id).textContent='₹0');
  document.getElementById('fullMonths').textContent='0';
  if(document.getElementById('calculatorArrears')) document.getElementById('calculatorArrears').textContent='₹0';
  return;
 }
 ...
 const daysDue=Math.max(0,daysInStartMonth-start.getDate()+1);
 const part=(start.getDate()===1)?pension:Math.round(pension/daysInStartMonth*daysDue);
 const full=Math.max(0,(monthEnd.getFullYear()-start.getFullYear())*12+monthEnd.getMonth()-start.getMonth()-(start.getDate()===1?0:1));
 const fullAmt=pension*full;
 const total=part+fullAmt;
 document.getElementById('partMonthAmount').textContent=money(part);
 document.getElementById('fullMonths').textContent=full;
 document.getElementById('fullMonthAmount').textContent=money(fullAmt);
 document.getElementById('totalArrears').textContent=money(total);
 if(document.getElementById('calculatorArrears')) document.getElementById('calculatorArrears').textContent=money(total);
}
['arrearsPension','arrearsStart','arrearsCutoff'].forEach(id=>document.getElementById(id).oninput=calculateArrears);
document.getElementById('arrearsCutoff').value=endOfCurrentMonthISO();
showView('calculator');
