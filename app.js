...
navItems.forEach(btn=>btn.onclick=()=>showView(btn.dataset.view));
...
function syncArrears(){document.getElementById('arrearsPension').value=Math.round(parseFloat(document.getElementById('monthlyPension').textContent.replace(/[₹,]/g,''))||0);document.getElementById('arrearsStart').value=document.getElementById('commencement').textContent==='—'?'':document.getElementById('commencement').textContent.split(' ').reverse().join('-');}
function calculateArrears(){
 const pension=+document.getElementById('arrearsPension').value||0,start=new Date(document.getElementById('arrearsStart').value+'T00:00:00'),cutoff=new Date(document.getElementById('arrearsCutoff').value+'T00:00:00');
 if(isNaN(start)||isNaN(cutoff)||!pension){['partMonthAmount','fullMonthAmount','totalArrears'].forEach(id=>document.getElementById(id).textContent='₹0');document.getElementById('fullMonths').textContent='0';return}
 const monthEnd=new Date(cutoff.getFullYear(),cutoff.getMonth()+1,0), daysInMonth=new Date(start.getFullYear(),start.getMonth()+1,0).getDate(), daysDue=Math.max(0,daysInMonth-start.getDate()+1), part=Math.round(pension/daysInMonth*daysDue), full=Math.max(0,(monthEnd.getFullYear()-start.getFullYear())*12+monthEnd.getMonth()-start.getMonth()-(start.getDate()===1?0:1)), fullAmt=pension*full;
 document.getElementById('partMonthAmount').textContent='₹'+part.toLocaleString('en-IN');document.getElementById('fullMonths').textContent=full;document.getElementById('fullMonthAmount').textContent='₹'+fullAmt.toLocaleString('en-IN');document.getElementById('totalArrears').textContent='₹'+(part+fullAmt).toLocaleString('en-IN');
}
['arrearsPension','arrearsStart','arrearsCutoff'].forEach(id=>document.getElementById(id).oninput=calculateArrears);
document.getElementById('arrearsCutoff').value=new Date().toISOString().slice(0,10);
showView('calculator');
